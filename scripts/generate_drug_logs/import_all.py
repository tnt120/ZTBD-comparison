...
from generate_drug_logs.import_postgres import import_postgres_drug_logs
from generate_drug_logs.import_mysql import import_mysql_drug_logs
from generate_drug_logs.import_mongo import import_mongo_drug_logs, get_mongo_operations
...
def import_all(records_num):
    ...
    try:
        time_taken_pg = import_postgres_drug_logs(len(records), records)
    ...
    try:
        time_taken_mysql = import_mysql_drug_logs(len(records), records)
    ...
    try:
        time_taken6 = import_mongo_drug_logs(
            ops,
            {
                "host": MONGO6_HOST,
    ...
    try:
        time_taken8 = import_mongo_drug_logs(
            ops,
            {
                "host": MONGO8_HOST,
