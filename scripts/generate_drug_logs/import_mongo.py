...
def get_mongo_operations(records):
    ...
    ops = {}
    for rec in tqdm(records, desc="Adapting records"):
        op = {
            "$push": {
                "drugs_logs": {
                    "drug_id": rec["drug_id"],
                    ...
                    "day": rec["day"],
                    "taken_time": rec["taken_time"],
                    "time": rec["time"],
                }
            }
        }
        if rec["user_id"] not in ops:
            ops[rec["user_id"]] = []
        ops[rec["user_id"]].append(op)
...
def import_mongo_drug_logs(operations, conn_params, db, title):
