...
def import_postgres_drug_logs(records_count, records=None):
    ...
    for rec in tqdm(records, desc="Inserting"):
        ...
        if len(batch) >= 10000:
            args_str = ",".join(
                cursor.mogrify("(%s,%s,%s,%s,%s,%s,%s)", b).decode("utf-8")
                for b in batch
            )
            cursor.execute(
                "INSERT INTO drugs_logs (id, created_at, day, taken_time, time, drug_id, user_id) VALUES "
                + args_str
            )
            conn.commit()
            batch = []
    if batch:
        args_str = ",".join(
            cursor.mogrify("(%s,%s,%s,%s,%s,%s,%s)", b).decode("utf-8") for b in batch
        )
        cursor.execute(
            "INSERT INTO drugs_logs (id, created_at, day, taken_time, time, drug_id, user_id) VALUES "
            + args_str
...
if __name__ == "__main__":
    ...
    time_taken = import_postgres_drug_logs(args.records_num)
