...
def after(db, conn):
    if db == "pg":
        revert_insert_postgres(conn, "parameters_logs", [log_id])
    elif db == "mysql":
        revert_insert_mysql(conn, "parameters_logs", [log_id])
    elif db in ["mongo6", "mongo8"]:
        revert_insert_mongo(conn, "users", [log_id], "parameters_logs")
